function [centers, index, options, post, errlog] = sp_kmeans(centers, data, options)
...
%   end
  ...
  %%%%%%%%%%%%%%%
  %[minvals, index] = pdist2(centres, data, 'euclidean','Smallest',1) ;
  [minvals, index] = lph_pdist2(centers, data);
  for j = 1:ncentres
      curr_idx   = find(index ==j);
      curr_num = length(curr_idx);
      if curr_num > 0
          centers(j, :) = sum(data(curr_idx, :), 1) / curr_num;
      end
  end
  %%%%%%%%%%%%%%%
  ...
  if n > 1
    ...
    if max(max(abs(centers - old_centres))) < options(2) & ...
...
function [minvals, index] = lph_pdist2(centres, data) 

    num_per_iter = 50000;
    num_iter = size(data, 1) / num_per_iter;

    minvals = zeros(size(data, 1), 1, 'single');
    index    = zeros(size(data, 1), 1, 'single');
    for i = 1:num_iter
